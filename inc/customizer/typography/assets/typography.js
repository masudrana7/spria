jQuery( document ).ready(function($) {
	...
	$('.google-fonts-list').on('change', function() {
		...
		var indexes = $.map(bodyfontcontrol.softcodersthemefontslist, function(obj, index) {
		...
		$.each(bodyfontcontrol.softcodersthemefontslist[index].variants, function(val, text) {
		...
		// Usc_post_date the font category based on the selected font
		$(this).parent().parent().find('.google-fonts-category').val(bodyfontcontrol.softcodersthemefontslist[index].category);

		softcodersthemeGetAllSelects($(this).parent().parent());
	});
	...
	function softcodersthemeGetAllSelects($element) {
		var selectedFont = {
