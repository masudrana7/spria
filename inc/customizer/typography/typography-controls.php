...
if ( class_exists( 'WP_Customize_Control' ) ) {
	...
	class Customizer_Google_Fonts_Controls extends WP_Customize_Control {
		...
		public function __construct( $manager, $id, $args = array(), $options = array() ) {
			...
			$this->fontList = $this->softcoderstheme_getGoogleFonts( 'all' );
			// Decode the default json font value
			$this->fontValues = json_decode( $this->value() );
			// Find the index of our default font within our list of Google fonts
			$this->fontListIndex = $this->softcoderstheme_getFontIndex( $this->fontList, $this->fontValues->font );
		}
		...
		public function enqueue() {
			wp_enqueue_script( 'softcoderstheme-select2-js', trailingslashit( get_template_directory_uri() ) . 'inc/customizer/typography/assets/select2.min.js', array( 'jquery' ), '4.0.6', true );
			wp_enqueue_script( 'softcoderstheme-typography-controls-js', trailingslashit( get_template_directory_uri() ) . 'inc/customizer/typography/assets/typography.js', array( 'softcoderstheme-select2-js' ), '1.2', true );
			wp_enqueue_style( 'softcoderstheme-typography-controls-css', trailingslashit( get_template_directory_uri() ) . 'inc/customizer/typography/assets/typography.css', array(), '1.1', 'all' );
			wp_enqueue_style( 'softcoderstheme-select2-css', trailingslashit( get_template_directory_uri() ) . 'inc/customizer/typography/assets//select2.min.css', array(), '4.0.6', 'all' );
		}
		...
		public function to_json() {
			parent::to_json();
			$this->json['softcodersthemefontslist'] = $this->fontList;
		}
		/**
		 ...
		 */
		public function softcoderstheme_getFontIndex( $haystack, $needle ) {
		 ...
		 */
		public function softcoderstheme_getGoogleFonts( $count = 30 ) {
