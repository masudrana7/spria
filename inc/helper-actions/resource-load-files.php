...
trait ResourceLoadFiles {
  ...
  public static function maybe_rtl( $file ){
    if ( is_rtl() ) {
      $file =get_template_directory_uri() . '/assets/css/rtl-css/' . $filename . '.css';
      return $file;
    }
    else {
  ...
  public static function get_img( $filename ){
    $path = '/assets/img/theme/' . $filename;
    return self::get_file( $path );
  }

  public static function get_css( $filename ){
    if ( is_rtl() ) {
      $path = '/assets/css/rtl-css/' . $filename . '.css';
    }
    else {
      $path = '/assets/css/customize/' . $filename . '.css';
    }
    return self::get_file( $path );
  }
